...
type ConfigFile struct {
	...
	RemoteServers []string `json:"remote"`
...
func (self *Config) Reload() (err error) {
	...
	if err != nil {
		glog.Infof("Reload %s failed: %s", self.Path, err)
	} else {
		glog.Infof("Reload %s", self.Path)
		self.mutex.Lock()
		self.File = file
		self.mutex.Unlock()
	}
...
func (self *Config) Load() (err error) {
	...
	// first time to load
	glog.Infof("Loading: %s", self.Path)

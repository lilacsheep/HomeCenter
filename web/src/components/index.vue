  ...
  <el-row :gutter="20" v-loading="loading">
    <el-tabs tab-position="left" @tab-click="tabClick">
      <el-tab-pane label="代理配置">
        <el-col :span="8">
            ...
            <el-table-column prop="operation" align="right">
              <template slot-scope="scope">
                ...
                <span v-else-if="scope.row.key === 'port'">
                  <el-button
                    @click.native="edit_server(scope.row)"
                    type="text"
                    size="small"
                    >编辑</el-button
                  >
          ...
          <el-card
            v-if="server.info.instances.length > 0"
          ...
          >
            ...
            <div v-for="(item, i) in server.info.instances" :key="i">
              ...
              <el-popconfirm
                title="是否将实例从代理池中移除？"
                @onConfirm="instance_remove_from_pool(item.id)"
              >
                <el-button
                  slot="reference"
        ...
        <el-col :span="16">
          ...
          >
            <el-table-column prop="address">
              <template slot="header" slot-scope="scope">
                <el-button
                  ...
                  @click="instaces.create.visit = true"
                  >新增实例</el-button
                >
              </template>
              <template slot-scope="scope">
                <el-button
                  type="text"
                  size="mini"
                  @click="edit_instance(scope.row)"
                >
                  {{
                    `ssh://${scope.row.username}@${scope.row.address}`
                  }}</el-button
                >
              </template>
            </el-table-column>
            ...
            <el-table-column prop="delay" label="延迟" width="100">
              <template slot-scope="scope">
                <el-tag
                  v-if="scope.row.delay < 100"
                  size="mini"
                  type="success"
                  effect="plain"
                  >{{ scope.row.delay }} ms</el-tag
                >
                <el-tag
                  v-else-if="scope.row.delay < 200"
                  size="mini"
                  type="warning"
                  effect="plain"
                  >{{ scope.row.delay }} ms</el-tag
            ...
            <el-table-column label="操作" width="100">
              <template slot-scope="scope">
                <el-popconfirm
                  title="是否将实例删除？"
                  @onConfirm="instance_remove(scope.row.id)"
                >
                  <el-button
                    slot="reference"
                    style="color: red"
                ...
                <el-popconfirm
                  ...
                  title="是否将实例加入代理池？"
                  @onConfirm="instance_into_pool(scope.row.id)"
                >
                  <el-button
                    slot="reference"
                    style="color: green"
      ...
      <el-tab-pane label="访问记录">
        <el-input
          size="small"
          placeholder="请输入内容"
          v-model="roles.logs.filter"
          style="width: 300px; float: right; margin-bottom: 10px"
        >
          <el-button
            @click="submit_roles_logs_filter"
            slot="append"
            icon="el-icon-search"
          ></el-button>
        </el-input>
        <el-table :data="roleLogs" stripe border size="mini">
          ...
          <el-table-column label="操作" fixed="right" width="100">
            <template slot-scope="scope">
              <el-popconfirm
                title="是否将站点加入负载策略？"
                @onConfirm="add_log_to_role(scope.row)"
              >
                <el-button
                  slot="reference"
                  style="color: red"
        ...
        <el-pagination
          ...
          :total="roles.logs.pagination.total"
          @current-change="role_logs_pagination_change"
        ></el-pagination>
      </el-tab-pane>
      <el-tab-pane label="规则配置">
        ...
        <el-input
          size="small"
          placeholder="请输入内容"
          v-model="roles.filter"
          style="width: 250px; float: right"
        >
          <el-button
            slot="append"
            icon="el-icon-search"
            @click="submit_roles_filter"
          ></el-button>
        </el-input>
            ...
            <template slot-scope="scope">
              <span v-if="scope.row.status">
                <el-tag effect="plain" size="mini"
                  >转发 -> {{ get_instance(scope.row.instance_id) }}</el-tag
                >
              </span>
              <el-tag v-else size="mini" type="danger" effect="plain"
                >封禁</el-tag
          ...
          <el-table-column label="操作" fixed="right" width="100">
            <template slot-scope="scope">
              <el-button
                ...
                size="mini"
                icon="el-icon-edit"
                @click="change_role(scope.row)"
              ></el-button>
              <el-popconfirm
                title="是否删除该规则？"
                @onConfirm="remove_role(scope.row)"
              >
                <el-button
                  slot="reference"
                  style="color: red"
        ...
        <el-pagination
          ...
          @current-change="roles_pagination_change"
        ></el-pagination>
      </el-tab-pane>
    </el-tabs>
    ...
    <el-dialog title="新增实例" :visible.sync="instaces.create.visit">
      <el-form :model="instaces.create.form" label-position="right">
        <el-form-item label="地址" label-width="100px">
          <el-input
            v-model="instaces.create.form.address"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="用户" label-width="100px">
          <el-input
            v-model="instaces.create.form.username"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="密码" label-width="100px">
          <el-input
            v-model="instaces.create.form.password"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="秘钥" label-width="100px">
          <el-input
            ...
            placeholder="请输入内容"
            v-model="instaces.create.form.private_key"
          ></el-input>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="instaces.create.visit = false"
          >取 消</el-button
        >
        <el-button size="small" type="primary" @click="submit_create_instance"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      <el-form :model="instaces.edit.form" label-position="right">
        ...
        <el-form-item label="地址" label-width="80px">
          <el-input
            v-model="instaces.edit.form.address"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="归属" label-width="80px">
          <el-select
            size="small"
            v-model="instaces.edit.form.country_code"
            filterable
            placeholder="请选择"
          >
            <el-option
        ...
        <el-form-item label="用户" label-width="80px">
          <el-input
            v-model="instaces.edit.form.username"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
        <el-form-item label="密码" label-width="80px">
          <el-input
            v-model="instaces.edit.form.password"
            size="small"
            autocomplete="off"
          ></el-input>
        </el-form-item>
    ...
    <el-dialog title="新增规则" :visible.sync="roles.create.visit">
      <el-form :model="roles.create.form" label-position="right">
        ...
        <el-form-item label="规则" label-width="100px">
          <el-switch
            v-model="roles.create.form.status"
            active-text="转发"
            inactive-text="封禁"
            @change="role_change"
          ></el-switch>
        ...
        <el-form-item
          label="实例"
          label-width="100px"
          :style="roles.create.style"
        >
          <el-select
            v-model="roles.create.form.instance_id"
            size="small"
            placeholder="请选择转发"
          >
            <el-option label="默认转发" value=""></el-option>
            <el-option
              v-for="(item, i) in instanceData"
              :key="i"
              :label="item.address"
              :value="item.id"
            ></el-option>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button size="small" @click="roles.create.visit = false"
          >取 消</el-button
        >
        <el-button size="small" type="primary" @click="submit_create_role"
          >确 定</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog title="修改规则" :visible.sync="roles.change.visit">
      <el-form :model="roles.change.form" label-position="right">
        <el-form-item label="规则" label-width="100px">
          <el-switch
            v-model="roles.change.form.status"
            active-text="转发"
            inactive-text="封禁"
            @change="role_change"
          ></el-switch>
        ...
        >
          <el-select
            v-model="roles.change.form.instance_id"
            size="small"
            placeholder="请选择转发"
          >
            <el-option label="默认转发" value=""></el-option>
...
export default {
  ...
  methods: {
    tabClick: function (tab, event) {
      if (tab.index === "0") {
        ...
        this.refresh_visit_logs(
          this.roles.logs.pagination.page,
          this.roles.logs.pagination.limit
        );
      } else if (tab.index === "2") {
        this.refresh_roles(
          this.roles.pagination.page,
          this.roles.pagination.limit
        );
      }
    },
    querySearch(queryString, cb) {
      var restaurants = this.dns_select;
    ...
    submit_create_instance: function () {
      ...
      this.$api
        .post("/proxy/instance/create", this.instaces.create.form)
    ...
    submit_edit_server() {
      let that = this;
      this.$api
        .post("/proxy/server/update", this.server.edit.form)
        .then(function (response) {
    ...
    submit_edit_instance() {
      let that = this;
      this.$api
        .post("/proxy/instance/update", this.instaces.edit.form)
        .then(function (response) {
    ...
    instance_remove_from_pool(id) {
      let that = this;
      this.$api
        .post("/proxy/instance/pool/remove", { id: id })
        .then(function (response) {
    ...
    instance_remove(id) {
      let that = this;
      this.$api
        .post("/proxy/instance/remove", { id: id })
        .then(function (response) {
          that.$message({ message: "移除成功", type: "success" });
          that.refresh_instances();
          that.refresh_server();
    ...
    instance_into_pool(id) {
      let that = this;
      this.$api
        .post("/proxy/instance/pool/add", { id: id })
        .then(function (response) {
    ...
    refresh_visit_logs(page, limit) {
      ...
      this.$api.get("/proxy/logs", params).then(function (response) {
        ...
        that.roles.logs.pagination.total = response.hasOwnProperty("count")
          ? response.count
          : 0;
      });
    },
    ...
    submit_create_role() {
      let that = this;
      this.$api
        .post("/proxy/role/add", this.roles.create.form)
        .then(function (response) {
          that.roles.create.visit = false;
          that.$message({ message: "修改成功", type: "success" });
          that.refresh_roles(
            that.roles.logs.pagination.page,
            that.roles.logs.pagination.limit
          );
        })
        .catch(function (response) {
          that.$message.error({ message: response.message, type: "error" });
    ...
    remove_role(item) {
      let that = this;
      this.$api
        .post("/proxy/role/remove", { id: item.id })
        .then(function (response) {
    ...
    submit_change_role() {
      let that = this;
      this.$api
        .post("/proxy/role/change", this.roles.change.form)
        .then(function (response) {
          that.$message({ message: "修改成功", type: "success" });
          that.refresh_roles(
            that.roles.logs.pagination.page,
    ...
    refresh_roles(page, limit) {
      ...
      this.$api.get("/proxy/roles", params).then(function (response) {
        ...
        that.roles.pagination.total = response.hasOwnProperty("count")
          ? response.count
          : 0;
      });
    },
